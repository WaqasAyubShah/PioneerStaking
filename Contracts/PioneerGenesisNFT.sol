...
/**
 * @title Pioneer Genesis NFT
 * @dev To facilitate the genesis sale for the Pioneer platform
 */
contract PioneerGenesisNFT is ERC721WithSameTokenURIForAllTokens("PioneerGenesis", "DXG") {
    using SafeMath for uint256;

    // @notice event emitted upon construction of this contract, used to bootstrap external indexers
    event PioneerGenesisNFTContractDeployed();
    ...
    // @notice event emitted when PioneerAccessControls is updated
    ...
    constructor(
        ...
        tokenURI_ = _tokenURI;
        emit PioneerGenesisNFTContractDeployed();
    }
    ...
    function buy() public payable {
        require(contribution[_msgSender()] == 0, "PioneerGenesisNFT.buy: You already own a genesis NFT");
        require(
            _getNow() >= genesisStartTimestamp && _getNow() <= genesisEndTimestamp,
            "PioneerGenesisNFT.buy: No genesis are available outside of the genesis window"
        );

        uint256 _contributionAmount = msg.value;
        require(
            _contributionAmount >= minimumContributionAmount,
            "PioneerGenesisNFT.buy: Contribution does not meet minimum requirement"
        );

        require(
            _contributionAmount <= maximumContributionAmount,
            "PioneerGenesisNFT.buy: You cannot exceed the maximum contribution amount"
        );

        require(remainingGenesisTokens() > 0, "PioneerGenesisNFT.buy: Total number of genesis token holders reached");
        ...
        require(fundsTransferSuccess, "PioneerGenesisNFT.buy: Unable to send contribution to funds multisig");
    ...
    function increaseContribution() public payable {
        require(
            _getNow() >= genesisStartTimestamp && _getNow() <= genesisEndTimestamp,
            "PioneerGenesisNFT.increaseContribution: No increases are possible outside of the genesis window"
        );

        require(
            contribution[_msgSender()] > 0,
            "PioneerGenesisNFT.increaseContribution: You do not own a genesis NFT"
        );
        ...
        require(
            ...
            "PioneerGenesisNFT.increaseContribution: You cannot exceed the maximum contribution amount"
        );
        ...
        require(
            ...
            "PioneerGenesisNFT.increaseContribution: Unable to send contribution to funds multisig"
        );
    ...
    function adminBuy(address _beneficiary) external {
        require(
            ...
            "PioneerGenesisNFT.adminBuy: Sender must be admin"
        );
        require(_beneficiary != address(0), "PioneerGenesisNFT.adminBuy: Beneficiary cannot be ZERO");
        require(balanceOf(_beneficiary) == 0, "PioneerGenesisNFT.adminBuy: Beneficiary already owns a genesis NFT");
    ...
    function updateGenesisEnd(uint256 _end) external {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "PioneerGenesisNFT.updateGenesisEnd: Sender must be admin"
        );
        // If already passed, dont allow opening again
        require(genesisEndTimestamp > _getNow(), "PioneerGenesisNFT.updateGenesisEnd: End time already passed");

        // Only allow setting this once
        require(!genesisEndTimestampLocked, "PioneerGenesisNFT.updateGenesisEnd: End time locked");
     ...
     */
    function updateAccessControls(PioneerAccessControls _accessControls) external {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "PioneerGenesisNFT.updateAccessControls: Sender must be admin"
        );
        require(address(_accessControls) != address(0), "PioneerGenesisNFT.updateAccessControls: Zero Address");
        accessControls = _accessControls;
    ...
    */
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        if (from != address(0) && _getNow() <= genesisEndTimestamp) {
            revert("PioneerGenesisNFT._beforeTokenTransfer: Transfers are currently locked at this time");
        }
    }
}